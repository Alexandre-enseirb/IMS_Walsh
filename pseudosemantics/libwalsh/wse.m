...
Xw_b = quantification(Xw, params.nBitsAmp, params.max_bin);
% Xw_b = Xw;
[len,~] = size(Xw_b);
mid = len/2;
...
%figure();
for i=1:nWindows
    sig_Walsh((i-1)*params.nCoeff+1:i*params.nCoeff) = Xw_b(:,i).'*params.W;
    %plot(params.time_axis(1:i*params.nCoeff), sig_Walsh(1:i*params.nCoeff)); grid on;
end
sig_Walsh = repmat(sig_Walsh(1:nWindows*params.nCoeff), params.osr, 1);
sig_Walsh = sig_Walsh(:);
end
