...
if sum(clustersSizes) ~= params.nCoeffs
...
fc = params.BW.middleFreq; % carrier

sig = getRandomConformedSignal(params, params.timeAxis);
nWindows = size(sig.Xw_b, 2);

stats.mem           = zeros(params.nCoeffs, nIter);
stats.summed_coeffs = zeros(params.nCoeffs, 1);
stats.mean_coeffs   = zeros(params.nCoeffs, 1);
stats.var_coeffs    = zeros(params.nCoeffs, 1);
stats.max_coeffs    = zeros(params.nCoeffs, 1);
stats.std_c         = zeros(params.nCoeffs, 1);
stats.mean_over_time = zeros(params.nCoeffs, nWindows);
% Stats
for i=1:nIter
    sig = getRandomConformedSignal(params, params.timeAxis);
