function [combinations] = improvedCoefficientsSelection(nCoefficientsToSelect, ...
    cluster, ...
    carrier, ...
    ...
    stopAtMax, ...
    params)
%IMPROVEDCOEFFICIENTSSELECTION generates combination of modulated Walsh coefficients
%
%   [combinations] = IMPROVEDCOEFFICIENTSSELECTION(nCoefficientsToSelect, cluster, nSymbolsCombinationsPerCoefficientsCombinations,
...
% On veut matcher nos tailles pour se comparer a tous les coefficients restants
powerSpectrumsIn = zeros(length(idxOut), params.Nfft);
for i=1:length(idxIn)
    powerSpectrumsIn = powerSpectrumsIn + repmat(powerSpectrums(idxIn(i), :), length(idxOut), 1);
end
