...
[carriers, stats] = generateWalshCarrierFixedDurationClusterize(params, 100, carrier_name, totalDuration, [4 16 44]);
...
Sks = {Sk Sk2 Sk3 Sk4};
sigRecdB = zeros(4, length(params.freq_axis));
for iSigns = 1:4
    Sk = Sks{iSigns};
    ...
    sigRecdB(iSigns, :)  = 10*log10(sigRecPow/max(sigRecPow));
    
    if ~isConform(sigRecdB(iSigns, nullFrequencyIdx:maxConformFrequency), params.BW_visible(nullFrequencyIdx:maxConformFrequency).')
        nNotConform = 1;
    end
    ...
    bitErrors(iSigns) = sum(symbolsTxBinary~=symbolsRxBinary, "all");
end
%% AFFICHAGE
