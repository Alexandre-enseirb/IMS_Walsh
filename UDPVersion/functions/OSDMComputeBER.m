...
function [BER, flag] = OSDMComputeBER(buffer1, buffer2, commParams)
...
% Extraction du preambule et estimation du meilleur temps symbole
preambleExtracted = sig(1:nSymbPreambleUpsample);
symbolTime = findBestSubSample(preambleExtracted, commParams.fse, nSymbPreamble, commParams.ModOrderQPSK);
...
interestingCoeffs3D = reshape(interestingCoeffs, walshParams.cluster2Size, [], commParams.Img.dataToTransmitIntensity);
...
imgInit = imread("Data/walsh.png");
imgInit = squeeze(imgInit(:,:,1));
imgInitVector = imgInit(:);

imgInitBinary = int2bit(imgInitVector, bpi, leftMSB);
    
bitError = sum(abs(ImgRxBinary-double(imgInitBinary)), "all");
BER = bitError/(numel(imgInitVector)*bpi) * 100;